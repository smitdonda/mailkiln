/**
 * `<MailForge>` — the editor.
 *
 * @module mailforge/react/MailForge
 */
...
import { MailForgeProvider } from './context.jsx'
import { applyTools } from './tools.js'
import { I18nProvider } from './i18n/index.jsx'
import { useMailForge } from './useMailForge.js'
 ...
 *   Enables the "Send test" button. mailforge renders the message; your app sends it.
...
export function MailForge({
  ...
  const store = useMailForge({ value, defaultValue, onChange, vars: vars ?? null })
  ...
  useEffect(() => {
    ...
    if (unknown.length) {
      ...
      console.warn(
        `mailforge: unknown theme ${unknown.length === 1 ? 'key' : 'keys'} ${unknown.join(', ')}. See the Theme type for valid keys.`,
      )
    }
  }, [theme])

  const resolvedAppearance =
    manualAppearance ?? (appearance === 'auto' ? (systemDark ? 'dark' : 'light') : appearance)
  ...
  return (
    <I18nProvider locale={locale} messages={messages}>
      <MailForgeProvider value={contextValue}>
        <div
          ref={rootRef}
          className={['mf-root', className].filter(Boolean).join(' ')}
          data-mf-theme={resolvedAppearance}
        ...
        >
          <DndRoot>
            <Toolbar
              ...
              onToggleAppearance={() =>
                setManualAppearance(resolvedAppearance === 'dark' ? 'light' : 'dark')
              }
            />
