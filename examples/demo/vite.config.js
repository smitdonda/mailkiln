...
// The playground consumes mailkiln straight from source so edits hot-reload.
...
export default defineConfig({
  ...
  // `prefix(mk)`. Without it the playground silently serves a stylesheet whose
  // Tailwind directives failed to parse — so `npm run dev` doubles as a check on
  // the real CSS pipeline.
  plugins: [react(), tailwindcss()],
  resolve: {
    alias: {
      'mailkiln/style.css': r('../../src/styles.css'),
      'mailkiln/core': r('../../src/core/index.js'),
      mailkiln: r('../../src/index.js'),
